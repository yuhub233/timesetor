...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  final _approachRateController = TextEditingController(text: '0.1');
  ...
  void _loadSettings() {
    ...
    if (settings.isNotEmpty) {
      ...
      _approachRateController.text = (settings['time_approach_rate'] ?? 0.1).toString();
    }
  }
  ...
  void dispose() {
    ...
    _approachRateController.dispose();
    super.dispose();
  }
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      await context.read<AuthProvider>().updateSettings({
        ...
        'time_approach_rate': double.tryParse(_approachRateController.text) ?? 0.1,
      });
      
      setState(() => _message = '设置已保存');
    } catch (e) {
      setState(() => _message = '保存失败: $e');
    }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    TextField(
                      controller: _approachRateController,
                      keyboardType: TextInputType.number,
                      decoration: const InputDecoration(
                        labelText: '时间靠拢速率 (0.05 - 0.5)',
                        border: OutlineInputBorder(),
                        helperText: '值越大，不常规时间向目标时间靠拢越快',
                      ),
                    ),
                    const SizedBox(height: 16),
                    SizedBox(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      '用户ID: ${context.watch<AuthProvider>().userId ?? "未知"}',
