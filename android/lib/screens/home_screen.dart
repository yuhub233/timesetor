...
class TimeDisplayScreen extends StatelessWidget {
                ...
                onPressed: () async {
                  ...
                  if (!result['success'] && context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(result['error'] ?? '记录起床失败')),
                    );
                  }
                },
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    if (confirmed == true) {
                      ...
                      if (!result['success'] && context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text(result['error'] ?? '记录睡觉失败')),
                        );
                      }
                    }
                  },
  ...
  Widget _buildSpeedBadge(double speed) {
    ...
    if (speed > 1.5) {
      ...
      color = AppConstants.entertainmentColor;
    } else {
      text = '正常';
      color = Colors.grey;
    }
  ...
  Widget _buildActivityIndicator(String activity) {
    ...
    final colors = {
      'rest': AppConstants.restColor,
      'entertainment': AppConstants.entertainmentColor,
      'study': AppConstants.studyColor,
    };
