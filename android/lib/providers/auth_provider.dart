...
class AuthProvider extends ChangeNotifier {
  ...
  Map<String, dynamic> _parseSettings(String? json) {
    if (json == null) return {};
    try { return Map<String, dynamic>.from(const JsonDecoder().convert(json)); } catch (_) { return {}; }
  }
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    try {
      ...
      notifyListeners();
      return {'success': true};
    } on ApiException catch (e) { return {'success': false, 'error': e.message}; } catch (e) { return {'success': false, 'error': '登录失败'}; }
  }
  
  Future<Map<String, dynamic>> register(String username, String password) async {
    try {
      final response = await ApiService.post('/auth/register', {'username': username, 'password': password});
      await _saveAuth(response);
      _isLoggedIn = true;
      notifyListeners();
      return {'success': true};
    } on ApiException catch (e) { return {'success': false, 'error': e.message}; } catch (e) { return {'success': false, 'error': '注册失败'}; }
  }
